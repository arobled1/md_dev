...
If we apply equation (\ref{eq:h_plusminus}) to a vector that is one state higher $\hat{a}\kat{\psi_n}$, we obtain
\be
...
\be
  H \hat{a}^{\dagger}\kat{\psi} = (E_n - \hbar \omega) \hat{a}^{\dagger}\ket{\psi}
\ee
...
When we consider that there is ground state $\ket{\psi_0}$ such that $\het{a}^{\dagger}\ket{\psi_0} = 0$, we can plug in the defintion for $\hat{a}^{\dagger}$ and solve for $\ket{\psi_0}$, giving
\be
