...
We can now plug our energies into equation (\ref{eq:harm_part_func}) for the partition function.
\be
  Q = \sum_k e^{-\beta \hbar \omega(n + \frac{1}{2})}
\ee
We can actually evaluate the sum to get a more compact form by remembering that $\frac{1}{1-x} = \sum_n x^n$ and that $\hbar, \omega, and \beta$, are just constants.
\be
  \begin{split}
    Q &= \sum_n e^{-\beta \hbar \omega(n + \frac{1}{2})} \\
    &= e^{-\frac{\beta \hbar \omega}{2}} \sum_n e^{- \beta \hbar \omega n} \\
    &= e^{-\frac{\beta \hbar \omega}{2}} \sum_n \Big( e^{- \beta \hbar \omega} \Big)^n \\
  ...
  \end{split}
\ee
We already know the energies of the harmonic oscillator, but we can derive the energies in the canonical ensemble with the partition function using the relationship,
\be
...
\be
  \begin{split}
    ...
    &= - \frac{\partial}{\partial \beta} \Big[ \ln1 - \ln \Big(2 \sinh(\frac{\beta \hbar \omega}{2})\Big) \Big] \\
  ...
  \end{split}
\ee
The equation for the total energy can be made exactly the same as in Tuckerman's book \cite{tuckerman} by expressing the $\tanh$ function in terms of exponentials.
\be
  \begin{split}
