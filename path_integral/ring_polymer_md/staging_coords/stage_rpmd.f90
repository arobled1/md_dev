...
! Compute the external force from quartic potential and add to the spring force
...
primitives_x = 0.0d0
! Reading in position samples (assuming in cartesian coordinates)
...
! Allocate and initialize bead primitive and staging velocities
allocate(primitives_v(pbeads,num_samples))
allocate(stage_vels(pbeads,num_samples))
primitives_v = 0.0d0
stage_vels = 0.0d0
! Getting initial primitive velocities from maxwell-boltzmann distribution
call random_seed(rando_seed)
do i = 1, num_samples
    call sample_velocities(primitives_v(:,i))
enddo

! Transform primitive velocities into staging velocities
...
! =============================================================================
! MD section
! Allocate array to save X(n delta t) for computing Kxx(n deltat)
...
open(30,file='general_stage_correlation.txt', action="readwrite")
