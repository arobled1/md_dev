...
In Quantum Statistical Mechanics, the partition function is written more formally as (see Appendix 3),
...
\be
  ...
  \end{split}
\ee
where the forces on the staging coordinates are defined recursively as,
\be
  \begin{split}
...
The virial energy,
\be
  \epsilon_{vir} = \frac{1}{P} \sum_{k=1}^P \Big[ \frac{1}{2}x_k \frac{\partial U}{\partial x_k} + U(x_k) \Big]
\ee
...
\ee
is added to velocity verlet so that a random kick is added to the system at each time step that depends on a random variable sampled from the normal distribution with mean $\mu = 0$ and standard deviation $\sigma = 1$.
Doing so gives an algorithm for sampling the canonical density.
Histograms of the positions and momenta for the oscillator should then mimic a gaussian distribution (Figure 5).
To be specific, a histogram of the position should mimic a marginal distribution in x, which can be done by integrating out the velocity in the canonical density.
\be
  P(x) = \sqrt{ \frac{m \omega^2}{2 \pi k_B T} } e^{ - \frac{m \omega^2 x^2}{2 k_B T}}
\ee
And a histogram of the velocity should mimic a marginal distribution in v, which can be done by integrating out the position in the canonical density.
\be
...
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{../../../langevin_thermo/figures/harmonic_pos_dens.pdf}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{../../../langevin_thermo/figures/harmonic_vel_dens.pdf}
    \end{subfigure}
    \caption{Histograms of position (left) and momenta (right) for the one dimensional harmonic oscillator ($m = 1$, $\omega = \sqrt{8}$). The friction $\gamma$ was set to 10 in this simulation. The histograms are compared to the marginal distributions in position and velocity (equations (23) and (24)). }
\end{figure}

\subsection{Path Integral Molecular Dynamics}

Finally, the PIMD code was written to replicate the results done by Tuckerman et al.\cite{berne}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=1.1]{Figures/pimd/virial.pdf}
    \caption{Plot of the instantaneous virial estimator along with cumulative averages along a PIMD trajectory. Since the simulation was performed in using atomic units, we defined variables $\hbar = 1$ and $m = 0.01$ so that $\omega = 3$ and $K_BT = 3/15.8$. Simulations were considered successful if the cumulative average of the virial estimator converged at $E_{vir} = \hbar \omega / 2 = 1.5$. This simulation converged at a virial energy value of 1.5006394 after 100,000 MD steps with $\Delta t = 0.01$.}
\end{figure}

The amount of time requried for convergence depends mostly on the choice of thermostat and integrator.
In the original code by Tuckerman et al the Nos\'e-Hoover chain method was used to thermostat every bead along with RESPA integrators for integrating the equations of motion.
Using a Langevin thermostat and simple velocity Verlet means that the convergence time will be much higher than if Nos\'e-Hoover chains with RESPA integrators were used.
In the original paper, the convergence
...
\ee
The raising operator is defined so that if it acts upon a state vector on energy level n, $\ket{psi_n}$, the result is a state vector at energy level n+1 with a factor,
\be
  \hat{a}^{\dagger}\ket{\psi_n} = \sqrt{n+1}\ket{\psi_{n+1}} .
\ee
Likewise, the lowering operator is defined so that if it acts upon a state vector at energy level n, $\ket{psi_n}$, the result is a state vector at energy level n-1 with a factor,
\be
  \hat{a}\ket{\psi_n} = \sqrt{n}\ket{\psi_{n-1}} .
\ee
...
\subsection{Harmonic Oscillator Position Distribution}
This section serves to derive the position distribution function for a harmonic oscillator in the canonical ensemble, equation (23).
We start with the canonical distribution function,
\be
  f(x,v) = \frac{e^{- \beta H}}{\int dx \int dv e^{- \beta H}}
\ee
and integrate out the velocity in the numerator so that we are left with a density in x.
This simply means that we can cancel out an integral in the denominator.
\be
  \begin{split}
    f(x) &= \frac{\int dv e^{- \beta H}}{\int dx \int dv e^{- \beta H}} \\
    &= \frac{\int dv e^{- \beta [\frac{1}{2}mv^2]} e^{- \beta [\frac{1}{2}m \omega^2 x^2]} }{\int dx \int dv e^{- \beta [\frac{1}{2}mv^2]} e^{- \beta [\frac{1}{2}m \omega^2 x^2]}} \\
    &= \frac{e^{- \beta [\frac{1}{2}m \omega^2 x^2]} }{\int dx e^{- \beta [\frac{1}{2}m \omega^2 x^2]}}\\
  \end{split}
\ee
Remembering that the integral goes over all space, we just have a simple gaussian integral.
\be
  \int_{-\infty}^{\infty} dx e^{- \beta [\frac{1}{2}m \omega^2 x^2]} = \sqrt{\frac{2 \pi}{\beta m \omega^2}} = \sqrt{\frac{2 \pi k_B T}{ m \omega^2}}
\ee
So the density becomes,
\be
  f(x) = \sqrt{ \frac{m \omega^2}{2 \pi k_B T} } e^{ - \frac{m \omega^2 x^2}{2 k_B T}} .
\ee
\subsection{Harmonic Oscillator Velocity Distribution}
This section serves to derive the velocity distribution function for a harmonic oscillator in the canonical ensemble, equation (23).
We start with the canonical distribution function, which is just a joint distribution in x and v,
\be
  f(x,v) = \frac{e^{- \beta H}}{\int dx \int dv e^{- \beta H}}
\ee
and integrate out the position in the numerator so that we are left with a density in v.
This simply means that we can cancel out an integral in the denominator.
\be
  \begin{split}
    f(v) &= \frac{\int dx e^{- \beta H}}{\int dx \int dv e^{- \beta H}} \\
    &= \frac{\int dx e^{- \beta [\frac{1}{2}m \omega^2 x^2]} e^{- \beta [\frac{1}{2}mv^2]} }{\int dx e^{- \beta [\frac{1}{2}m \omega^2 x^2]} \int dv e^{- \beta [\frac{1}{2}mv^2]}} \\
    &= \frac{ e^{- \beta [\frac{1}{2}mv^2]} }{\int dv e^{- \beta [\frac{1}{2}mv^2]}}\\
  \end{split}
\ee
Remembering that the integral goes over all space, we just have a simple gaussian integral.
\be
  \int_{-\infty}^{\infty} dv e^{- \beta [\frac{1}{2}mv^2]} = \sqrt{\frac{2 \pi}{\beta m}} = \sqrt{\frac{2 \pi k_B T}{ m }}
\ee
So the density becomes,
\be
  f(v) = \sqrt{ \frac{m}{2 \pi k_B T} } e^{ - \frac{m v^2}{2 k_B T}} .
\ee
\bibliography{mybib}
